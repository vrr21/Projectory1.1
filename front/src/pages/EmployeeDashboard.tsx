...
import { message, ConfigProvider, theme, Select, Row, Col } from 'antd';
...
const EmployeeDashboard: React.FC = () => {
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      fetchTasks();  // Обновление задач после успешного запроса
    } catch (error) {
                  ...
                  {teams
                    .flatMap((team) =>
                      team.members.filter((member) => {
                        // Фильтруем сотрудников по выбранной команде или если это сам сотрудник
                        return member.id === user?.id || team.ID_Team === selectedTeam;
                      })
                    )
                    .map((member) => (
